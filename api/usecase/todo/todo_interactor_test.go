...
func TestAdd(t *testing.T) {
	...
	cases := []struct {
		...
		prepareStoreMockFn func(m *mock_usecase.MockTodoRepository, args domain.Todo)
	...
	}{
		{
			...
			// prepareTrasMockFn: func(m *mock_transaction.MockSqlHandler, mk *mock_usecase.MockTodoRepository, args domain.Todo) {
			// 	m.EXPECT().DoInTx(func(tx *sql.Tx) (interface{}, error) {
			// 		mk.EXPECT().Store(args).Return(nil).AnyTimes()
			// 		return nil, nil
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.prepareStoreMockFn(TodoRepository, tt.args)
